...
const Fl_init_all = new CFunction(LIBCFLTK, 'Fl_init_all', null, 'void');
const Fl_register_images = new CFunction(LIBCFLTK, 'Fl_register_images', null, 'void');
const Fl_lock = new CFunction(LIBCFLTK, 'Fl_lock', null, 'int');
const Fl_Window_new = new CFunction(LIBCFLTK, 'Fl_Window_new', null, 'pointer', 'int', 'int', 'int', 'int', 'string');
const Fl_Button_new = new CFunction(LIBCFLTK, 'Fl_Button_new', null, 'pointer', 'int', 'int', 'int', 'int', 'string');
...
function main() {
    Fl_init_all.invoke();        // init all styles
    Fl_register_images.invoke(); // necessary for image support
    Fl_lock.invoke();            // necessary for multithreaded support
    let w = Fl_Window_new.invoke(100, 100, 400, 300, "Hello ...");
    let b = Fl_Button_new.invoke(160, 210, 80, 40, "Click me");
